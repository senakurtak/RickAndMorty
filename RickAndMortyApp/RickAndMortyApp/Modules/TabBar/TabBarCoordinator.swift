    ...
    override func start() -> Observable<Void> {
        ...
        let coordinates = navigationControllers.enumerated()
            .map { (offset, element) -> Observable<Void> in
                ...
                switch items {
                case .character:
                    return coordinate(to: CharacterCoordinator(rootViewController: self.rootViewController, navigationController: element) )
                case .episode:
                    return coordinate(to: EpisodeCoordinator(rootViewController: self.rootViewController, navigationController: element))
                case .location:
                    return coordinate(to: LocationCoordinator(rootViewController: self.rootViewController, navigationController: element))
                }
            }
